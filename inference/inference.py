...
class BCSInferenceEngine:
    ...
    def __init__(
        self,
        classifier_weights: Union[str, Path] = "E:/ai_bcs_estimation/ReNet18_ordinal_classification_model/model_results/best_ordinal_resnet18.pth",
        detector_weights: Optional[Union[str, Path]] = "E:/ai_bcs_estimation/anatomical_region_ditector/runs/detect/anatomical_region_yolo11m/weights/best.pt",
    ...
    def _load_classifier(self) -> OrdinalResNet18:
        ...
        model = OrdinalResNet18(trainable_layers=trainable_layers, num_thresholds=4)
    ...
    def predict_crop(
        ...
        probs = torch.sigmoid(logits).squeeze(0)  # Shape: [4]

        # Standard Ordinal prediction: count thresholds passed (> 0.5)
        # Class index = sum(probs > 0.5) => integer 0 to 4
        predicted_idx = int((probs > 0.5).sum().item())
        predicted_bcs = INDEX_TO_BCS.get(predicted_idx, float(predicted_idx))

        # Expected value continuous score (smooth interpolation)
        ...
        certainty = float(torch.abs(probs - 0.5).mean().item() * 2 * 100)

        return predicted_bcs, round(certainty, 2), threshold_details
